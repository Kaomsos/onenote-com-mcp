...
- Page Copy/Move preserves the source title unless `destination_title` is explicitly supplied. For `semantic_content_v1`, each Page result includes content-free source-to-transform and transform-to-target diagnostics so a fidelity failure can be localized without exposing Page content or raw XML.
