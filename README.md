...
- **COM Convergence & Reconciliation:** Mutations wait for stable live observations, preserve typed HRESULT/retryability, and classify applied/not-applied/partial/indeterminate state before returning.
- **Operation Control Plane:** All 56 production operations are bound once in a canonical Registry and run through `OperationRuntime`; there is no production advanced profile. Read, Mutation, Lifecycle, Filesystem Effect, and UI Effect keep distinct completion semantics while sharing coordination, deadline, backend-call accounting, content-free audit, and stable execution outcomes.
- **Mutation Attempt Control:** Title/content edits, Rename, supported Reorder, Reparent, typed Delete, and Close use explicit policies for one bounded principal backend attempt plus content-free outcome and recovery fields. Every currently registered attempt executes at most once; the Runtime composes this 029 primitive into operation-wide outcomes and Copy/Move sagas rather than maintaining a second attempt model.
- **Process-local Coordination:** Reads may share execution, while each OneNote mutation/lifecycle operation holds one exclusive lease from confirmation through COM execution and stable read-back; this does not claim a cross-process or Desktop-wide transaction.
...
Every success and failure envelope includes an additive, content-free `execution` projection with the operation, final stage, effect kind, backend category, attempt/replay state, backend-call count, retry safety, recovery action, and cache generation. See the canonical [Operation Runtime design](docs/design/operation_runtime.md). It never contains Page content, raw XML, binary, bridge payload, secrets, original arguments, object IDs, or full paths.
...
* `list_notebooks` is the only hierarchy List tool and returns all currently open Notebook roots as `items/count` without filters or pagination. Exact `get_*` tools remain available for one-object metadata.
* `expand_notebook(id)` / `expand_section_group(id)` stop at Section leaves; `expand_section(id)` / `expand_page(id)` rebuild complete Page indentation subtrees. `expand_hierarchy(root_id, max_depth=8, include_recycle_bin=false)` provides the generic numeric-depth view. Every Expand returns the same recursive `tree={item,children[]}` schema and reads hierarchy metadata only. Exact-root relationship validation is isolated to that root's Notebook, so an unrelated open Notebook cannot poison the requested tree; malformed relationships inside the target Notebook still fail closed.
...
* `get_path` / `get_parent`: Read stable COM container ancestry; Page indentation browsing is provided by Expand.
...
* `sync_notebook`: Submit synchronization for a typed Notebook target. COM proves request acceptance only, so the response keeps `complete=false` and `completion_observable=false`.
...
* `copy_page` / `copy_section` / `copy_section_group` / `copy_notebook` and the three typed Move tools are each one public mutation call. The Runtime holds one exclusive operation boundary while `CopyService` rebuilds a live, content-aware internal plan before any target mutation. Agents submit intent, exact IDs, scope, and confirmation fields; they never carry `plan_digest`, operation tokens, allocated IDs, completed steps, or replay state between calls.
* The old `plan_copy`, `plan_move_page`, `plan_move_section`, and `plan_move_section_group` tools are absent from every production profile with no compatibility aliases. This release does not expose Preview either; no `preview_copy`/`preview_move` capability exists, and normal execution never depends on one.
...
* Every successful `reparent_*`, `copy_*`, and `move_*` execution returns top-level `destination_position`, an observed post-operation position for the response's target root. Page uses a zero-based index in the destination Section's complete flat Page sequence; Section and SectionGroup use same-type direct children of the final parent. Even when a Page subtree is included, only the target root position is returned and no Page level is included. Notebook Copy returns `status="not_applicable"`. Execution never performs an implicit Reorder to manufacture a requested position.
* Page Copy and Page Move both default to the single selected Page. Pass `include_descendants=true` to the execution tool only when the complete indentation subtree is intended; that scope is included in the same call's live internal plan. For a root-only Move, excluded descendants are internally bound and promoted one level before the selected source is recycled, so they remain active in the source Section. `destination_section_id` always names a Section rather than a parent Page: Copy preserves existing Page order, appends the newly allocated target block, normalizes its root to level 1, and restores selected descendant levels relative to that root. Section, SectionGroup, and Notebook Copy remain fully recursive. Copy never overwrites, merges, or auto-renames a conflicting target.
...
Migration note: Page Copy/Move omission now means root Page only. To select the complete indentation subtree, submit `include_descendants=true` directly to the single Copy/Move call. Older Plan/Execute clients must remove the Plan step and `plan_digest`; there is no compatibility alias or replacement state token.
