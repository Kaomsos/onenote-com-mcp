"""Human-gated bootstrap phase shared by unified interactive scenarios."""
...
async def run_interactive_bootstrap_phase(
    scenario: Scenario,
    ...
    *,
    ...
    """Authoring confirmation, detection, and template identity freeze for one run."""
    ...
    synthetic_content_only = bool(getattr(recipe, "synthetic_content_only", True))
    source_structure = (
    ...
    declared_roles = tuple(role.role for role in recipe.cache_identity.notebook_roles)
    ...
    if not isinstance(notebook_paths, dict):
        notebook_paths = {"source": manifest["disposable_targets"]["source_notebook_path"]}
    if not (
    ...
    if callable(freeze_structures):
        ...
        for role in declared_roles:
            ...
            if not isinstance(frozen_structure, Mapping) or set(frozen_structure) != set(
                declared_structure
            ):
                raise InvariantFailure(
    ...
    response = (
        await _bounded_input(
            f"Type {verdict!r} to record the human UI verdict: ",
            args.interactive_timeout,
        )
    ).strip()
    if response != verdict:
    ...
    frozen_role_structures = {
        role: {key: dict(value) for key, value in observations[role].build.structure.items()}
        for role in declared_roles
    }
    manifest["role_structures"] = frozen_role_structures
    manifest["structure"] = {
        key: value for role in declared_roles for key, value in frozen_role_structures[role].items()
    }
    ...
    if len(declared_roles) == 1:
        fixture_validation["checks"] = list(fixture_result["validation"].get("checks", []))
...
class InteractiveBootstrapScenarioMixin:
    """Shared CLI flags for scenarios that include a fresh bootstrap phase."""
