...
- Materialized working Notebook 必须在 exact plain working tree 内有界打开声明的 SectionGroup 和 `.one` Section。全部 child 路径必须在第一次 child COM 调用前完成预算、containment、reparse 与 typed parent 校验并冻结；同一 role 优先在一个 PowerShell/COM session 内按 parent-before-child 顺序批量激活，并在该 session 末尾尝试读取一次 pages hierarchy。逐项 `OpenHierarchy` 错误最多只重试该失败项一次；成功返回 ID 的容器不得仅因同会话 snapshot 暂不可见而立即重开，末尾 hierarchy 读取错误必须保留逐项结果并交给后续 Fixture observer 收敛。Notebook 直属 child 使用绝对 working path 与空 relative ID；SectionGroup 下的嵌套 child 只能使用文件名与同批或先前 snapshot 已证明的精确 parent ID。不得组合绝对 path 与非空 parent ID，也不得在 OneNote 开始接管路径后重新按磁盘存在性决定剩余请求。仅打开空 Notebook shell、只收到 `OpenHierarchy` 返回 ID 或单次 snapshot 均不算成功；后续仍须按 Notebook-relative typed address 连续两次稳定并完成完整 live validation。已观察到的确定性类型、parent、path、回收站或唯一性冲突必须立即 fail closed。旧逐对象 fallback 只供不支持 batch 的纯测试 fake 使用，不属于真实 cache runtime。若且唯若全部未激活尝试都已证明 exact parent、没有类型/打开冲突并返回 `0x8004201D`，fixture 阶段可在 mutation 前关闭并重开同一 working path 一次；必须保留初次证据，且绝不能复制第二份 working bundle、修改模板或重放 mutation。OneNote 重建 ID 时，必须先按唯一 Notebook-relative typed address 记录 old→live 映射，后续 validator/mutation 只能使用 live ID。一次恢复仍失败时必须保留现场与绑定实际 live ID 的 lease，但不能反向污染已验证的 immutable template；通用 failure finalizer 默认随后精确关闭该 lease，显式 keep 模式才保持 active。映射缺失、歧义或 live validation 失败时 exact cache entry 必须变为不可命中的 invalid/quarantine，模板与失败现场保持不删除。
