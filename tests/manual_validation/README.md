...
`move-page` 的 root-only 源由 recipe v7 自动创建为标题 `01-Readback / Page\\:  %~界` 的 RichText + 3 列 × 2 行 Table；fixture 回读必须证明完整 `Outline/RichText/Table` projection、精确表格拓扑及有限正列宽。该 case 省略 `destination_title`，要求 source→transformed→target 两段标题检查均为 true 后才允许非永久删源。第二个 subtree case 自动创建无 Table/binary 的 RichText 父子页，要求两页逐一以完整 `semantic_content_v1` tier 通过，再执行显式改名与完整父子范围 Move。场景仍验证范围、`id_map`、排除后代提升及删除顺序，且不替代 `copy-page` 的 Image/List/Tag/Equation 富内容矩阵。
...
[TODO 040](../../docs/todo/040_move_readback_validation_followups.md) 的真实后端回归归属于三个现有的自动 fixture scenario，而不是 `interactive-move-page-content`：`hierarchy-navigation` recipe v4 自动创建特殊字符标题 Page，并以 exact ID 验证 `get_hierarchy_path.path_segments`；`copy-page` recipe v14 在 cross-Notebook root-only case 省略 `destination_title`，验证默认标题的 source→transformed→target 三段回读；`move-page` recipe v7 自动创建同标题 RichText + 3 列 × 2 行 Table，以及独立的纯 RichText subtree，逐页要求 `semantic_content_v1`，只有完整 Copy contract 通过后才非永久删源。三个场景都自行创建 disposable fixture，不要求手工编辑或预建 cache：
