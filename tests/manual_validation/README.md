...
`--use-cache` 只改变 fixture 来源：validated hit 把关闭的 immutable template opaque-copy 到本次 run 的 role-specific working 路径。每个 role 先用第一组 import identity 按精确 parent 批量 `OpenHierarchy`；全部请求成功后，runner 执行 `CloseNotebook(force=false)` 并证明 exact ID/path 已关闭，再从同一个 working path 只重开 Notebook shell。第二组 identity 才是 mutation identity：重新枚举完整 hierarchy，按 Notebook-relative typed address 重绑全部 SectionGroup、Section 与 Page ID，要求连续两次结构稳定，然后每个 Page 只读取一次完成完整内容验证。`cache-working-import-checkpoint.json` 保存两组 ID、逐 role 路径、batch/close 证据、import/close/reopen 耗时，并固定 `mutation_attempted=false`；`cache-materialization.json` 将 materialize 决策与真正的 validated-hit/cold-build/bootstrap 来源分开记录，`cache-hierarchy-convergence.json` 则细分 hierarchy/content 耗时。Template 永不打开或修改；第二次 identity 建立前不会执行 mutation。Cache 仍不保存 working lease，也不与 run 维持所有权或生命周期关系；多个 run 可从同一 immutable entry 得到各自唯一的 working paths。Programmatic miss 仍先构建并 live-validate fresh bundle、精确关闭、发布 immutable template，再对新 materialized working copy执行相同 import checkpoint。静态声明的 `reparent-page` v3、最小单 Section 的 `create` v3 和用 typed sentinel Section 固化空 anchor Group 的 `copy-section-group` v5，在 publish 前还执行 Recipe persistence checkpoint；旧 recipe fingerprint 自动 miss。任一 import close/reopen、ID rebind、双稳定或内容验证失败均保留 working files/evidence；默认 failure finalizer 精确关闭第二次 lease，显式 keep 才保持打开。
...
所有 materialized working copy 都无条件执行一次 import checkpoint，不再只把 close/reopen 当作 `0x8004201D` 后的补救。第一次 identity 完成 parent-aware batch activation 后精确关闭；第二次从同一路径重开，只由完整 hierarchy、typed ID 重绑、双稳定和单次 Page 内容读取建立 mutation identity。初次 batch 证据仍保存在 `materialized-hierarchy-open[-<role>].json`，两次 identity 与 close receipt 统一写入 `cache-working-import-checkpoint.json`。它不会复制第二份 working bundle、修改或重建 cache template，也不会重放 mutation；任一步失败继续 fail closed，并按默认 failure lifecycle 精确关闭当前 Notebook。
