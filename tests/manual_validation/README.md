...
| `delete` | Delete-Sandbox 下准备独立 Page、Section 与 SectionGroup target；分别用原 `delete_page/delete_section/delete_section_group` 的 `items` 模式执行非永久 Delete 并核对回收站状态；永久删除关闭；`--keep-worksite` 保持 Notebook 打开并记录精确目标 |
