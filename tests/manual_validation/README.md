...
`--use-cache` 只改变 fixture 来源：validated hit 把关闭的 immutable template opaque-copy 到本次 run 的 role-specific working 路径。每个 role 先用第一组 import identity 按精确 parent 批量 `OpenHierarchy`；全部请求成功后，runner 执行 `CloseNotebook(force=false)` 并证明 exact ID/path 已关闭，再从同一个 working path 只重开 Notebook shell。第二组 identity 才是 mutation identity：重新枚举完整 hierarchy，按 Notebook-relative typed address 重绑全部 SectionGroup、Section 与 Page ID，要求连续两次结构稳定，然后每个 Page 只读取一次完成完整内容验证。`cache-working-import-checkpoint.json` 保存两组 ID、逐 role 路径、batch/close 证据，并固定 `mutation_attempted=false`。Template 永不打开或修改；第二次 identity 建立前不会执行 mutation。Cache 仍不保存 working lease，也不与 run 维持所有权或生命周期关系；多个 run 可从同一 immutable entry 得到各自唯一的 working paths。Programmatic miss 仍先构建并 live-validate fresh bundle、精确关闭、发布 immutable template，再对新 materialized working copy 执行相同 import checkpoint。`reparent-page` v3 对 publish 前的 fresh fixture 继续保留自身持久化 checkpoint，但 validated hit 不再依赖 recipe 特例。任一 import close/reopen、ID rebind、双稳定或内容验证失败均保留 working files/evidence；默认 failure finalizer 精确关闭第二次 lease，显式 keep 才保持打开。
