...
- `Rich-Page` / `01-Readback / Page\\:  %~界`（父页）：基础内容为已确认的 `Outline/RichText/Table/Image`；无公式页面使用 `semantic_content_v1`，分别验证有效标题、富文本、表格、非空 Outline、对象类型与 binary hash，并只容忍 TODO 037/UT-009 冻结的三类 COM 规范化。其中 `copy-page` 的特殊字符父页额外包含一个行内 Presentation MathML 公式和一个 `display="block"` 单行公式；行内公式仍属于 `RichText`，单行公式分类为 `DisplayEquation`，整页因此使用 `semantic_display_equation`；
...
`move-page` 的 root-only 源由 recipe v8 自动创建为标题 `01-Readback / Page\\:  %~界` 的 RichText + 3 列 × 2 行 Table；fixture 回读必须证明完整 `Outline/RichText/Table` projection、精确表格拓扑及有限正列宽。Destination 预置与源根同标题的一级 Page，以及标题为 `03-Subtree` 的一级 Page，另保留 00/99 位置锚点。root-only case 省略 `destination_title`，要求 source→transformed→target 两段标题检查均为 true、target 为 fresh allocated ID 且同标题 anchor 不变后，才允许非永久删源。第二个 subtree case 自动创建无 Table/binary 的 RichText 父子页，要求两页逐一以完整 `semantic_content_v1` tier 通过，再以源标题作为显式 `destination_title` 做完整父子范围 Move。每个 case 还在自身 `capture_snapshot` 上挂本地 observer，把 Page 根 `dateTime` 投影为 `page-datetime-<case>.json`（仅 Page ID、`YYYY-MM-DDTHH:MM:SSZ` 与同秒结论），并用该独立证据断言 source/target 同一 UTC 秒；`copy_report.page_results[].date_time.status` 只对照生产声明。场景仍验证范围、`id_map`/`allocated_ids`、排除后代提升、可观察非永久删除与回收站字段一致，且不替代 `copy-page` 的 Image/List/Tag/Equation 富内容矩阵。默认 `move-page` 与 `all` 行为不变；它不接受负向验证专有参数。

`negative-move-page-datetime-drift` 是独立、fresh-only、未进入 `all` 的负向场景。它使用自己的 disposable 双 Notebook fixture，只跑声明的 subtree source；在 Copy 最终 `dateTime` 校验之后、Move 删源前重校验之前，把范围内 child Page 根 `dateTime` 精确推进一个 UTC 自然秒，预期结构化 `copy_only`、source 保留、目标只一份、零删除，且不得把原 `copy_report` 改写为 `source_drifted`。仓库规则禁止把 `copy_only` 当成功，因此真实命令必须非零退出；验收看 `datetime-drift-negative.json` 的 `negative_gate_verified=true`、`run-failure.json` 中的预期 `copy_only`，以及 exact lifecycle 关闭证明。该场景不支持 `--use-cache`。
...
`copy-page` 是一个 `source`/`destination` 双 Notebook bundle。六个 case 的 before/after snapshot 显式打开 Page XML observer，把范围内 Page 根 `dateTime` 投影为 `page-datetime-<case>.json`（仅 Page ID、`YYYY-MM-DDTHH:MM:SSZ` 与同秒结论），并用该独立证据断言 source/target 同一 UTC 秒；`copy_report.page_results[].date_time.status` 只对照生产声明。Section / SectionGroup / Notebook Copy 不启用该 projection，证据形状不变。早期 recipe 在严格 Parent 中新增一个行内公式和一个独立单行公式，并要求回读恰好两个 MathML root、一个 `display="block"` 与两个规范 namespace 声明。`run-2026-08-12-01-11-36` 与 `run-2026-08-12-01-25-30` 证明 MathML 本身保持，但单行公式前出现空白；后续 detector/capture 又把该差异定位为 OneNote 在 DisplayEquation 前写入纯空白 `span + br`。v9 首次进入完整 `all --use-cache` 时，`run-2026-08-12-15-54-16` 与单独复跑 `run-2026-08-12-16-11-26` 都证明把 display marker 与公式放在同一次 append 会被真实 COM 合并，旧 detector 对“公式独占一个 OE、前驱 OE 非空”的布局假设不成立。当前 recipe v10 先写入普通富内容、行内公式和 marker，再用独立 append 写入 block MathML；fixture 门要求公式所在文本没有可见残留，并精确观察到当前环境已验证的一个 `span + br` 前置空行，不再依赖 OE 独占或前驱 OE。`run-2026-08-12-16-30-58` 已证明 v10 fixture/cold publish/materialization 成功，首个 Copy 目标的拓扑、正文、对象、binary 和两条公式语义也保持；失败只剩清除已知 span 后的页面 canonical 差异。增强诊断后的 `run-2026-08-12-16-44-30` 进一步证明 source/target 条件包装数同为 2，并把首差异定位到 formula-only Outline 的 `Size.width/height`。Comparator 现在只规范化完整配对公式条件包装，以及节点集合受限、恰好一个完整 block 公式且没有正文/其他 markup 的独立 Outline 派生 Size；Position、混合内容 Outline 和普通/不完整注释继续 fail closed。Copy 发送前仍清除已知空白包装并使用 `semantic_display_equation`，行内公式继续作为 RichText 的有界 MathML 严格比较。v10 fingerprint 使旧 cache 不会命中；独立 `copy-display-equation` 的三跳真实运行已经证明空白包装规范化稳定，formula-only Size 规则和完整 `copy-page` 六 case 后续也已由真实 run 闭合。六个 case 继续覆盖同 Section、同 Notebook 跨 Section、跨 Notebook 三种目标范围，各自再覆盖 root-only 与完整子树。当前 recipe v16 还在跨 Section destination 预置与源根同标题及仅大小写不同的一级 Page，在跨 Notebook destination 预置与源根同标题的一级 Page；same-section root-only 与 cross-Notebook root-only 省略 `destination_title`，cross-section root-only 使用源标题的 casefold，cross-Notebook subtree 使用源标题作为显式 `destination_title`。每个 case 都要求 `allocated_ids` 与 `id_map` 指向 fresh target，且与 source/anchor IDs 不相交，保持 Parent/Child 相对层级以及全部 collision anchor 的正文、order、level 与 parent。默认按反向 case 顺序清理六个根目标并验证两个 Notebook 恢复；`--keep-worksite` 则保留全部目标供 UI 对照。Recipe v12 还会在 fresh fixture 阶段从 `get_page_content_objects` 选择唯一 Image 的公开 `id`，调用 `get_page_content_object_binary(page_id, page_content_object_id)`，只记录媒体类型、解码字节数与 SHA-256，绝不保存 Base64 或内部 `callback_id`；该证据已由后续 v13 和最终批次真实 run 闭合。

2026-08-21：v15 的两次 fresh run 在 fixture 阶段暴露了 casefold lookup 对两个预置 root-title anchor 的错误复用；v16 对这两个 anchor 使用公开 `create_page` 路径，并验证返回 Page ID 相对 fixture preflight 和第一个 anchor 均为 fresh。此改动尚未经过新的真实 run。

TODO 040 的 recipe v14 首次真实复验 `run-2026-08-18-19-34-01`，随后 `run-2026-08-18-19-36-50` 再次复现同一首 case 失败：目标已创建且正文可见文本、对象集合、Image binary 与两条 MathML 语义投影全部一致，但 transformed/target 的 `Page/Title/OE` 具有相同 `alignment/quickStyleIndex/style` canonical 属性名集合，而整个属性元组的 canonical hash 不同；现有 content-free 证据不声称三个值都分别发生变化。旧诊断因此误报 `DisplayEquation`。第一次修复增加了正确的 `PageTitle` typed 诊断，但 `run-2026-08-18-19-47-32` 证明 eligibility 错把 canonical comparator 本来已排除的 generated/volatile identity 属性也算进属性集合，得到 `title_text_equal=true`、OE `1/1`、canonical 首差异仅 `style`，却仍 `attribute_sets_equal=false`。当前实现改为复用 canonical 有效属性集合，不新增忽略项；只在 title 文本、Title 结构、OE 数量和该集合全部相等时 pairwise 归一化三个 COM 派生值，并以 `title_oe_com_style_normalization` 返回 content-free 计数/属性名。有效属性增删、正文 OE style、标题文本及其他 Page 内容仍由负合同 fail closed。

`run-2026-08-18-19-54-39` 随后证明该 comparator 修正有效：同 Section root/subtree、跨 Section root/subtree 四个 case 全部继续执行，默认标题的 cross-Notebook root-only 也返回 `verified/lossless/copy_contract_satisfied=true`、目标 item 精确保留 `01-Readback / Page\\:  %~界`，且 DisplayEquation comparator 全部通过。场景只因该独立 tier 的 Page result 没有仅供 `semantic_content_v1` 使用的 `semantic_content_stages` 而 fail closed；两个 Notebook 均精确关闭。生产报告现新增所有 tier 共用、并纳入 lossless gate 的 `title_readback_stages`，保留 `semantic_content_stages` 的原边界；场景改为验证该通用链，不降低 Copy fidelity。四次失败均未删源。

修复后的用户前台 `run-2026-08-18-20-03-25` 已完整通过 recipe v14：六个 same/cross Section/Notebook root-only/subtree case 全部 `verified/lossless/copy_contract_satisfied=true`，零 issue、零 skipped content，9 个 fresh target 与 source/anchors 不相交，所有 collision anchor 保持不变。每个 Page 的 `title_readback_stages` 两段均通过且 content-free；cross-Notebook root-only 明确省略 `destination_title` 并回显 `title_override_requested=false`，默认特殊字符标题精确保真。父页的 DisplayEquation、Image binary、Table 与 RichText 检查以及子页 List/Tag 检查全部通过。默认清理反向非永久删除 9 个目标并得到 `restored=true`，source/destination 两个 Notebook 均精确关闭，filesystem working evidence 未删除；scenario 只启动一个 MCP process。
...
所有新建的 cache、`.s-<16 hex>` publish staging、`.m-<16 hex>` materialize staging、working copy、inventory/artifact 和 JSON/XML 原子临时路径都使用普通绝对 Windows 路径，并在 copy、atomic publish 或 OneNote open 前完成 240 UTF-16 code units preflight。传给 OneNote COM 用于 Notebook root create/open 的 working 路径另受真实 Fresh 双 Notebook create 证据支持的 147-unit 安全兼容上限；run identity 会按实际 run root 动态把 working leaf 压缩到 12–64 units，无法容纳最短 digest 名时在零 COM 调用前 fail closed。Role 最多 12 字符，run evidence leaf 最多 64 units，opaque relative path最多 96 units/8 层；项目不依赖 `LongPathsEnabled`，不使用 `\\?\`。Opaque tree 每层先预算子路径再 `stat`/进入/读取，避免无界扫描先触发裸 `WinError 3`；authored working bundle 的 live projection 同时核对完整 64-hex digest 和 24-hex instance key。Maintenance 在取得只读 COM snapshot 前预算自身将生成的 open lock、marker、receipt、summary、必要 index 与原子临时路径，但把已经存在的 managed payload 当作 cleanup 输入：路径预算不会成为删除门限，ownership、固定根 containment、plain-tree/reparse、实际打开状态和交互确认仍全部 fail closed。新建路径预算失败仍以非零 `path_budget_exceeded` 返回 phase、target、limit/actual/over-by、触发路径、零/已发生副作用、`failure_evidence_written` 和 typed remediation；`WinError 3` 不进入仅面向 `WinError 5/32` 的状态守卫重试。
...
八个 `interactive-<operation>` 场景（5 个 Interactive Copy、`interactive-move-page-content`、`interactive-move-page`、`interactive-user-authored-fixture`）是 HUMAN-GATED、排除于 `all` 的统一入口。不带 `--use-cache` 时，同一次 run 按六阶段串联 `notebook → bootstrap → fixture → scenario → report → lifecycle`：bootstrap 创建 fresh disposable Canvas/authoring zone 或整页 intake Section、写 run-bound checkpoint、以有界 timeout 等待用户本人完成受控 authoring/整页转入并给出精确 verdict，成功后精确关闭 authored bundle、发布 immutable template，并在 fixture 阶段 materialize 第二份 working copy完成 live validation；scenario 阶段才执行 Copy/Move 或一致性校验。带 `--use-cache` 时跳过 bootstrap，按五阶段直接从 ready template materialize 新 working copy；cache miss、歧义、evidence-only 或 invalid entry 一律 fail closed，提示不带 `--use-cache` 重新 authoring，不得隐式重建。Fresh 路径禁止传入 `--template-instance-id`；cache 路径可显式传入，或在恰好存在一个 ready、mutation-eligible 且 fingerprint 匹配的 instance 时自动选择。`--keep-worksite` 只保留 scenario working bundle 并跳过契约内 restore/cleanup；fresh 路径的 template 发布是继续执行的前提，不再由 keep-worksite 抑制。Agent、pytest、CI、hook 和后台进程不得执行这些真实命令。
...
`interactive-copy-inline-equation` 用于对照 block MathML 空行问题。Fresh 路径在 bootstrap 阶段自动构建相同的 Source Parent 富文本/表格/图片基线，并通过受限 HTML 写入一个无 `display` 属性、前后均有普通正文的 Presentation MathML；用户不编辑 Page，只确认自动 fixture 在一行内正常显示。Detector 固定要求恰好一个完整 MathML、同一 `OE` 内存在可见前后文（可为同一个 `T`，也可为 OneNote 规范化产生的 `T(before) → T(math) → T(after)`）、零 `display` 属性和零相关 `<br>`。Copy 阶段要求 source/target 都继续满足相同 inline 门禁，任何公式周围新增 `<br>` 都单独失败；配合 `--capture-page-xml` 可直接保存两侧 XML。

```powershell
.venv\Scripts\python.exe tests\manual_validation\run.py interactive-copy-inline-equation --dry-run --json
.venv\Scripts\python.exe tests\manual_validation\run.py interactive-copy-inline-equation
...
TODO 004 的首次真实取证必须逐类型串行执行，任一非零结果立即停止并保留现场。InkDrawing 先以 fresh 路径发布精确 fixture，再以 `--use-cache` 做一次 Copy：

```powershell
.venv\Scripts\python.exe tests\manual_validation\run.py interactive-copy-ink-drawing --dry-run --json
.venv\Scripts\python.exe tests\manual_validation\run.py interactive-copy-ink-drawing
...
.venv\Scripts\python.exe tests\manual_validation\run.py interactive-copy-media-file
...
UI Shape 使用一个小矩形作为固定 fixture；不要在 authoring 阶段改用箭头，因为箭头的 `AnchorPoint` 结构属于另一种形状变体：

```powershell
.venv\Scripts\python.exe tests\manual_validation\run.py interactive-copy-ui-shape --dry-run --json
.venv\Scripts\python.exe tests\manual_validation\run.py interactive-copy-ui-shape
...
`interactive-user-authored-fixture` 是 bounded UserAuthored 的统一入口，不进入 `all`。Recipe v4 将 `mutation_eligible` 与 `move_source_deletion_allowed` 固化进 authored cache entry，并在 materialized identity gate 复核；旧 v3 fingerprint 不再命中。Authored publisher 只接受显式安全矩阵：`ready` 必须 `mutation_eligible=true`，而 Move 源删除资格由 Recipe 独立声明为 true 或 false；`evidence_only` 的两项资格必须都是 false。Fresh 路径在同一次 run 内完成 authoring zone 创建、用户编辑、detector/validator、template 发布与 live revalidation；`--use-cache` 可显式传入 `--template-instance-id authored-<24 hex>`，或在恰好存在一个 ready、mutation-eligible 且 fingerprint 匹配的 instance 时自动选择。缺失、格式错误、未知实例、歧义以及 `evidence_only` 都在 working Notebook 打开前 fail closed；fresh 产生的 `evidence_only` 只形成只读取证结果，必须报告 `mutation_eligible=false`。该能力当前定位为足够开发取证使用的临时脚手架；完整 authoring-zone、多实例和状态真实矩阵已作为低优先级 [TODO 020](../../docs/todo/020_user_authored_fixture_development_scaffold.md) 单独维护，不再阻塞 TODO 014 或生产 Copy/Move。

`interactive-move-page` v3 是“整页 fixture intake + 真实跨 Notebook Move”入口，不进入 `all`。Fresh scaffold 创建 source 与 destination 两个 disposable Notebook role：source 把 `01-Whole-Page-Intake-Placeholder` 留在只读系统说明 Section，并提供空的 `01-Whole-Page-Intake`；destination 提供 exact `01-Move-Destination/99-Destination-Anchor`。用户必须通过 OneNote Desktop 的“移动或复制”对话框，把一个完整、非敏感、可丢弃且包含正文作者/修订标记的 root leaf Page 转入 source intake。不得在 placeholder canvas 上重建内容，也不得把业务 Page 的唯一副本交给验证；若原 Page 需要保留，应先由用户制作 disposable duplicate。确认后 detector 要求 intake 中恰好出现一个不同于 placeholder 的新 Page ID，并把 `source_canvas_page` 从 scaffold ID 精确重绑定到该整页 ID；不按标题、路径或最近修改时间猜测。此场景显式选择敏感的本地 revision evidence：按文档顺序记录每个正文 `author*` / `lastModifiedBy*` marker 的节点 ordinal、节点类型、属性名、原始作者/initials/resolution ID 值及其 SHA-256，并标记 `marker_values_exposed=true`、`author_metadata_exposed=true`、`sensitive_evidence=true`。它仍不保存 Page 标题、正文或 raw XML；其他 scenario 的通用 snapshot 默认继续 hash-only。只能转入非敏感 disposable Page，并按敏感本地 artifact 管理整个 run 目录。

完整 typed projection、代表性受支持内容、revision marker、双 role opaque inventory 与 materialized live identity 全部通过后才发布 `ready`；v3 fingerprint 不复用旧 v1/v2 entry。随后 scenario 对新物化的 source working Page 调用且只调用一次公开 `move_page(include_subpages=false)`，不传 `destination_title`。静态权限为 Create + Writes + Deletes，公开 allowlist 只有 `move_page` 而没有独立 `delete_page`；`move_source_deletion_allowed=true` 只授权生产 Move 在 `verified=true`、`lossless=true`、`copy_contract_satisfied=true` 后非永久删除该 disposable working source，不授权打开或删除 cache template、用户外部来源页或 Notebook 文件。`revision-marker-comparison.json` 在成功与已有 target 的 `copy_only` 失败路径都按 exact target ID 逐 marker 记录 source-before/target-after 的原始作者元数据和摘要，并比较 ordinal、revision-node ordinal、节点类型、属性名、原值和 SHA-256；它是敏感本地诊断，不是成功门禁。成功路径在生产 Move 的 Copy/删除/目标拓扑门通过后继续请求 run-bound `ACCEPT ... MovePage MOVE`，即使 revision marker 漂移也会在 acceptance/result 中如实记录。`copy_only`、source 未删或人工拒绝仍失败并保留现场。

```powershell
.venv\Scripts\python.exe tests\manual_validation\run.py interactive-move-page --notebook-label whole-page-intake --dry-run --json
.venv\Scripts\python.exe tests\manual_validation\run.py interactive-move-page --notebook-label whole-page-intake
.venv\Scripts\python.exe tests\manual_validation\run.py interactive-move-page --use-cache --template-instance-id authored-<24 hex> --notebook-label whole-page-intake --dry-run --json
.venv\Scripts\python.exe tests\manual_validation\run.py interactive-move-page --use-cache --template-instance-id authored-<24 hex> --notebook-label whole-page-intake
```

`interactive-move-page-content` 是 [TODO 039](../../docs/todo/039_interactive_real_page_move_lossless_validation.md) 建立的双 Notebook human-gated 开发场景，不进入 `all`。Fresh 路径在 bootstrap 阶段创建 source instructions、一个 exact leaf Canvas，以及独立 destination Section/anchor；用户只能在 Canvas 中制作或粘贴非敏感的代表性内容。Recipe v11 接受 typed projection 明确观察到的有效 `RichText`，也接受 List/Tag、Table、Image 等其他已验证结构化能力；只有 `Outline` 的未编辑纯占位形状仍不能发布 ready 模板。v11 将 `mutation_eligible` 与 `move_source_deletion_allowed` 固化进 authored cache entry 并在 materialized identity gate 复核，因此旧 v10 fingerprint 不再命中。该 identity 在 template/working inventory byte-for-byte 相等、typed rebind、双稳定和 live validation 都已通过后，使用 materialization-stable 64-hex projection：保留可见文本、Table 列数与 Row/Cell 拓扑、List/Tag、对象计数和 binary hash，但忽略 OneNote 打开 opaque copy 时可自动重写的富文本展示 span/style 及 Table/Column/Row/Cell 展示属性；不放宽内容能力、标题、拓扑或后续 Move 的严格 Copy-before-delete fidelity gate。Canvas 标题可以改为代表性 Page 标题；确认后其 live title/path 会作为 authored template identity 冻结，后续 materialize 必须按该冻结地址重绑。完整 typed projection 才发布 `ready`；unknown/incomplete 内容只能成为 `evidence_only`，不能进入 Move。发布会先精确关闭 source/destination 两个 role，再把完整 bundle 作为 immutable template 发布并在 fixture 阶段 materialize 第二份 working bundle 做 live validation。成功 fresh run 会在终端展示已发布 `template_instance_id`，提示后续可用 `--use-cache` 复用该 template。该场景不是 TODO 040 自动回归的验收入口，不得改造成 programmatic fresh scenario。

```powershell
.venv\Scripts\python.exe tests\manual_validation\run.py interactive-move-page-content --notebook-label move-page-content --dry-run --json
.venv\Scripts\python.exe tests\manual_validation\run.py interactive-move-page-content --notebook-label move-page-content
.venv\Scripts\python.exe tests\manual_validation\run.py interactive-move-page-content --use-cache --template-instance-id authored-<24 hex> --notebook-label move-page-content --dry-run --json
.venv\Scripts\python.exe tests\manual_validation\run.py interactive-move-page-content --use-cache --template-instance-id authored-<24 hex> --notebook-label move-page-content --keep-worksite
```

Consumer 只允许一次公开 `move_page(include_subpages=false)`，不传 `destination_title`，从而同时验证默认标题保真；静态权限为 Create + Writes + Deletes，既不启用 Organize、Local File IO、Permanent Delete，也不增加 Move-only 内容 allowlist。`copy_only` / `copy_unverified` 时必须证明 source 仍 active，保留未验证 target 和 `lossless-diagnostic.json`，不询问人工 ACCEPT、不补调 mutation；适用 `semantic_content_v1` 时，诊断从生产 `page_results[*].semantic_content_stages` 保存 source→transformed 与 transformed→target 的完整性、check、摘要 hash、有界 mismatch path、typed content-object failures 及 Table width threshold/relative delta，且不保存标题、正文、style 值、原始 width、raw XML 或 binary。成功路径还会独立按 exact target ID 验证 destination Section 和 title 与 source 完全一致。只有生产报告同时满足 `verified=true`、`lossless=true`、`copy_contract_satisfied=true`、target title fidelity 且已非永久删源后，才要求用户检查 exact target 标题/内容并输入 run-bound `ACCEPT ... MovePageContent`。Agent、pytest、CI 或后台任务不得执行上述两条真实命令。

`interactive-copy-inserted-file` 是 HUMAN-GATED 的 Copy 验证 Scenario，不进入 `all`。Fresh 路径在同一次 run 内完成 Canvas authoring、发布与 Copy 验证；`--use-cache` 命中后 materialize 全新的 working copy，显式加载层级、重绑定 live ID、重跑 `InsertedFile` detector/projection，然后执行一次同 Section root-only `copy_page`。机器门要求 source/target 都精确观察到一个公开 `kind=InsertedFile`、稳定对象签名一致、可见文本/内容对象/strict canonical read-back 通过且没有 omitted content；当前公开 XML 没有内联 `Data`，因此普通 binary SHA-256 项只记录其 absence，用户还必须实际打开目标附件并确认其合成文件内容一致，再输入 run-bound `ACCEPT ... InsertedFile COPY`。Copy plan 优先使用仍存在的 `pathSource`，否则回退到可读 `pathCache/path`；全部不可读时在创建目标前 fail closed，普通 evidence 不保存实际路径。场景没有 Delete 权限，不会删除源。缺少可命中的 `ready` entry 时会在 Notebook/MCP 启动前返回 `interactive_cache_miss` 并提示不带 `--use-cache` 重新 authoring，而不是隐式重建：

```powershell
.venv\Scripts\python.exe tests\manual_validation\run.py interactive-copy-inserted-file --dry-run --json
.venv\Scripts\python.exe tests\manual_validation\run.py interactive-copy-inserted-file
...
### TODO 040 自动 Page readback 回归

[TODO 040](../../docs/todo/040_move_readback_validation_followups.md) 的真实后端回归归属于三个现有的自动 fixture scenario，而不是 `interactive-move-page-content`：`hierarchy-navigation` recipe v4 自动创建特殊字符标题 Page，并以 exact ID 验证 `get_hierarchy_path.path_segments`；当时的 `copy-page` recipe v14 在 cross-Notebook root-only case 省略 `destination_title`，验证默认标题的 source→transformed→target 三段回读；当时的 `move-page` recipe v7 自动创建同标题 RichText + 3 列 × 2 行 Table，以及独立的纯 RichText subtree，逐页要求 `semantic_content_v1`，只有完整 Copy contract 通过后才非永久删源。当前 `copy-page` recipe v16 与 `move-page` recipe v8 在此基础上增加目标 Section 同标题根 Page 回归，见 [TODO 050](../../docs/todo/050_page_copy_move_duplicate_root_title.md)。三个场景都自行创建 disposable fixture，不要求手工编辑或预建 cache：
...
[TODO 043](../../docs/todo/043_copy_move_source_timestamp_fidelity.md) 的跨秒 Move 负向删源保护由独立场景执行、不进入 `all`，必须由用户本人前台单独运行；预期非零退出，验收看证据而不是退出码：

```powershell
.venv\Scripts\python.exe tests\manual_validation\run.py negative-move-page-datetime-drift
```

2026-08-23 用户前台运行 `run-2026-08-23-00-43-39` 取得预期负向证据：`negative_gate_verified=true`，source `dateTime` 精确跨一个 UTC 自然秒后 Move 返回 `copy_only`，source 保留、两页 target 唯一、原 Copy 报告仍为 verified/lossless/contract satisfied，`delete_hierarchy=0`，双 Notebook failure lifecycle 均精确关闭。该非零 run 已由用户确认为通过，并闭合 TODO 043。

这些真实命令只能由用户本人在前台运行；它们不读取 stdin，全部验收由机器 gate 完成。需要保留打开的现场时可以显式追加 `--keep-worksite`。Agent、pytest、CI 与后台任务只可执行相应 `--dry-run`。

2026-08-18 用户已在前台完成该矩阵：`hierarchy-navigation` 的 `run-2026-08-18-19-33-05`、`copy-page` 的 `run-2026-08-18-20-03-25`，以及 `move-page` 的 `run-2026-08-18-19-39-02` 与独立复跑 `run-2026-08-18-19-49-05` 均通过。三者完全使用自动 disposable fixture；`interactive-move-page-content` 未参与本轮验收，现保持 fresh/cache unified authored-template 开发约定。
...
<!-- dry-run-case: prob-timestamp-fidelity.default -->
```powershell
.venv\Scripts\python.exe tests\manual_validation\run.py prob-timestamp-fidelity --dry-run --json
```
...
<!-- dry-run-case: negative-move-page-datetime-drift.default -->
```powershell
.venv\Scripts\python.exe tests\manual_validation\run.py negative-move-page-datetime-drift --dry-run --json
```
...
<!-- dry-run-case: interactive-user-authored-fixture.default -->
```powershell
.venv\Scripts\python.exe tests\manual_validation\run.py interactive-user-authored-fixture --dry-run --json
```

<!-- dry-run-case: interactive-move-page.default -->
```powershell
.venv\Scripts\python.exe tests\manual_validation\run.py interactive-move-page --dry-run --json
```
...
当前安全门先于本节的 parent-aware 实现。用户真实 runs 已观察到：opaque-copy working Notebook 的 Group 子 Section 激活可能使 `ONENOTE.EXE` 在 `onmain.dll` 中访问冲突退出，也可能得到 Group/Section ID 后仍在完整有界窗口内缺失声明 Page；根因及 persistent client 因果均未确定。因此，上述 12 个公开 Recipe 和未注册的 `reorder-section-group` Recipe 均拒绝 `--use-cache`，不得以 sleep、重试、自动重启 OneNote 或猜测 ID/path 绕过。

对仍允许 cache 的 Recipe，Notebook 直属 child 使用 absolute working path 与空 relative ID，并在同一个显式配置的 PowerShell/COM client session 中执行 manifest-bound bounded activation。代码仍保留 SectionGroup child 的 parent-relative、parent-before-child 实现，供 TODO 052 隔离调查；但当前 programmatic shape gate 保证这些 Recipe 不会到达该路径，保留实现不能解释为嵌套 child 已安全。
...
`onenote-convergence` 另外保存 `restoration-comparison.json`。它对 hierarchy、exact IDs、page semantic/persistence identity、revision marker、content-object、capability 与 MathML projection 做严格比较；OneNote 对未触及 anchor Page 的非语义 XML 重序列化只记录为 `strict_page_hash_drift`，不会单独判定恢复失败。共享 `assert_restored` 仍保持严格 hash 比较，供其他场景使用。
...
唯一 MCP 子进程同时完成该 scenario 的最小 fixture、所选 mutation、before/after/restored 回读和契约内 restore/cleanup。它启动时使用 `scenarios/common/specs.py` 中固定的完整闭包 policy 和 tool allowlist，并在 fixture 创建前用 `health_check` 精确核对 policy、timeout 和 Copy budget；启动后不得扩权。Scenario MCP child 与 lifecycle wrapper 必须接收同一个显式、静态的 `persistent_powershell` adapter（写入 child 静态 env 与 wrapper 构造参数），不得只依赖继承的父进程 `LOCAL_ONENOTE_BRIDGE_ADAPTER`；dry-run 计划输出最终 `bridge_adapter`。Runner 不使用所有 scenario 的权限并集。Registry 会在导入注册时把每个 programmatic fixture 的 `creation_tools` 映射到必需 policy gate；例如 `create_section` 要求 Create，`create_page` 要求 Create + Writes，`add_page_image_from_file` 要求 Writes + Local File I/O，缺任一 gate 都会在创建 Notebook 或启动 MCP 前直接拒绝。Interactive/UserAuthored unified 场景的静态 allowlist 同时覆盖 fresh scaffold 与 scenario mutation；`--use-cache` 只跳过 bootstrap，不会在运行时扩权。
...
`move-page` 的语义是对显式范围执行重建。`include_subpages=false` 只选择根 Page；`include_subpages=true` 选择完整缩进子树。场景固定使用两个 Notebook，并只覆盖 `cross-notebook-root-only` 与 `cross-notebook-subtree`：不再重复同 Notebook 跨 Section，因为该位置变化已经由 typed `reparent-page` 验证。默认双 case 与 `all` 行为不变。跨秒删源保护由独立的 `negative-move-page-datetime-drift` 场景验证，不向 `move-page` 增加参数或权限。

Move 场景不负责附件、墨迹、形状或媒体的独立保真取证；这些结论由 Copy 场景及其逐类型 comparator 负责。为覆盖 TODO 040，root-only fixture 会自动创建特殊字符标题的 RichText/三列表格，并省略 `destination_title`；场景要求生产 Copy 报告 `verified/lossless/copy_contract_satisfied=true`，且 source→transformed 与 transformed→target 的 title check 均通过。随后验证实际 `id_map` 与选定范围一致并审计安全删除：root-only 只允许删除根 Page，并要求被排除子页先整体提升一级且保持 ID、Section、相对层级和内容；subtree 必须按叶到根删除父子两页。任何 Copy、标题/内容回读、提升、快照重校验或删除证据不完整都非零退出并保留现场。
