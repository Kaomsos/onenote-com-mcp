...
| `delete` | Delete-Sandbox 下准备两个带子页的 Page 根、Section 与 SectionGroup target；以测试 Batch effective Page 上限 5 证明 Notebook 总 Page 数超限时混合 `include_subpages=false/true` Page batch 仍可非永久 Delete：前者保护并提升排除子页，后者删除完整子树；同时证明含六 Page 的 Section scope 在 mutation 前拒绝且 snapshot 不变；三类成功批次均核对 `final_hierarchy` 与回收站状态；永久删除关闭；`--keep-worksite` 保持 Notebook 打开并记录精确目标 |
