...
`--use-cache` 只改变 fixture 来源：validated hit 把关闭的 immutable template opaque-copy 到本次 run 的 role-specific working 路径；cache build 已负责生成并验证 immutable template 的权威内容基线。每个 role 先用第一组 import identity 按精确 parent 批量 `OpenHierarchy`；全部请求成功后，runner 执行 `CloseNotebook(force=false)` 并证明 exact ID/path 已关闭，再从同一个 working path 只重开 Notebook shell。Import/close/reopen materialization checkpoint 只做 content-free 的 exact identity 与轻量层级证明。第二组 identity 才是 mutation identity：重新枚举完整 hierarchy，按 Notebook-relative typed address 重绑全部 SectionGroup、Section 与 Page ID，要求连续两次结构稳定，然后只捕获一次完整 `scenario before` snapshot。它同时完成 materialized 内容真实性复核和 mutation 基线取证，并通过 exact role/Notebook ID/digest 单次 handoff 给 scenario；全部 role 未消费完时，首次 mutation 会在 MCP 调用前 fail closed。每个 Page 的 hash、能力与 normalized object evidence 都从同一次 `get_page_xml(page_info=all)` 本地派生，不再为对象投影重复调用 `get_page_objects`。Cached manifest 中明确属于旧 run 的 `notebook_copy_root`、逐 role working Notebook path 和 lifecycle lease 会在先证明同源关系后按字段重绑到当前 run，并记录 `cache-run-local-path-remap.json`；不会递归替换内容，也不会修改模板，旧字段缺失或彼此不一致时 mutation 前 fail closed。`cache-working-import-checkpoint.json` 保存两组 ID、逐 role 路径、batch/close 证据、import/close/reopen 耗时，并固定 `mutation_attempted=false`；`cache-materialization.json` 将 materialize 决策与真正的 validated-hit/cold-build/bootstrap 来源分开记录，`cache-hierarchy-convergence.json` 细分 hierarchy/`scenario before` 耗时，`scenario-before-snapshot-handoff.json` 记录单次消费状态。Template 永不打开或修改；第二次 identity 建立前不会执行 mutation。Cache 仍不保存 working lease，也不与 run 维持所有权或生命周期关系；多个 run 可从同一 immutable entry 得到各自唯一的 working paths。Programmatic miss 仍先构建并 live-validate fresh bundle、精确关闭、发布 immutable template，再对新 materialized working copy执行相同 import checkpoint。静态声明的 `reparent-page` v3、用 sentinel Page 固化最小 Section 的 `create` v4 和用 typed sentinel Section 固化空 anchor Group 的 `copy-section-group` v5，在 publish 前还执行 Recipe persistence checkpoint；旧 recipe fingerprint 自动 miss。任一 import close/reopen、ID rebind、双稳定、内容验证或 handoff 失败均保留 working files/evidence；默认 failure finalizer 精确关闭第二次 lease，显式 keep 才保持打开。
...
所有 materialized working copy 都无条件执行一次 import checkpoint，不再只把 close/reopen 当作 `0x8004201D` 后的补救。第一次 identity 完成 parent-aware batch activation 后精确关闭；第二次从同一路径重开，materialization 本身只确认 exact identity 与轻量层级。随后由 typed ID 重绑、双稳定和唯一一次 `scenario before` 内容读取建立 mutation identity；该 snapshot 既是 live Recipe 真实性复核，也是 scenario 的 before baseline。初次 batch 证据仍保存在 `materialized-hierarchy-open[-<role>].json`，两次 identity 与 close receipt 统一写入 `cache-working-import-checkpoint.json`，单次消费状态写入 `scenario-before-snapshot-handoff.json`。它不会复制第二份 working bundle、修改或重建 cache template，也不会重放 mutation；任一步失败继续 fail closed，并按默认 failure lifecycle 精确关闭当前 Notebook。
...
| `reparent-page` | **typed 实验工具 / 当前 v3 fresh 与 cold-build cache 真实验证通过 / 已注册到 `all`**。通过 `reparent_page` 提交精确 ID 与 confirmation，不要求 Raw XML。唯一的跨 Section case 将 `01-Source-Section/01-Reparent-Page` 改属预置两个根 Page anchors 的 `02-Destination-Section`；目标 Page 同页包含 Rich Text、Table、List、Tag、Image。Recipe v3 在 fresh/cold-build 首次 mutation 或 template publish 前强制 close/reopen、ID/evidence 重绑和完整 live validation；生产工具冻结完整基线后只执行一次 Reparent mutation。2026-08-13 用户运行的 fresh 与 `--use-cache` cold-build 均完成 Page/内容对象 ID remap、富内容/无关对象验证、默认恢复和 lifecycle close；cache template 未打开且 inventory 不变。默认使用新 ID 逻辑移回，或由 `--keep-worksite` 保留。 |
