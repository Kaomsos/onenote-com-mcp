...
`bootstrap-move-page-content-fixture` 与 `interactive-move-page-content` 是 [TODO 039](../../docs/todo/039_interactive_real_page_move_lossless_validation.md) 专用、均不进入 `all` 的双 Notebook human-gated 场景。Bootstrap 创建 source instructions、一个 exact leaf Canvas，以及独立 destination Section/anchor；用户只能在 Canvas 中制作或粘贴非敏感的代表性内容。Recipe v7 接受 typed projection 明确观察到的有效 `RichText`，也接受 List/Tag、Table、Image 等其他已验证结构化能力；只有 `Outline` 的未编辑纯占位形状仍不能发布 ready 模板。Canvas 标题可以改为代表性 Page 标题；确认后其 live title/path 会作为 authored template identity 冻结，后续 materialize 必须按该冻结地址重绑。RichText/List/Table/Image 内容与对象身份统一由 Copy 回读语义投影派生的 persistence digest 负责：它只扁平化 OneNote 重开时可能合并的 Outline 容器边界，仍保留全部 OE 顺序、正文、有效富文本、List/Tag、Table、Image binary 和其他语义；生产 Copy equivalence 继续保留 Outline 分组比较。空 Outline/OE、布局元数据和对应原始对象种类序列不会单独否决 identity；其他能力类型继续使用严格的稳定正文摘要与对象种类 multiset fallback。完整 typed projection 才发布 `ready`；unknown/incomplete 内容只能成为 `evidence_only`，不能进入 Move。发布会先精确关闭 source/destination 两个 role，再把完整 bundle 作为 immutable template 发布并 materialize 第二份 working bundle做 live validation。成功发布 ready 模板后，普通终端输出会明确打印 `template_instance_id` 及一条可复制的、仍显式传入该 ID 的 `interactive-move-page-content` 命令；`--json` 保持单一 JSON 文档。
...
Consumer 只允许一次公开 `move_page(include_subpages=false)`，不传 `destination_title`，从而同时验证默认标题保真；静态权限为 Create + Writes + Deletes，既不启用 Organize、Local File IO、Permanent Delete，也不增加 Move-only 内容 allowlist。`copy_only` / `copy_unverified` 时必须证明 source 仍 active，保留未验证 target 和 `lossless-diagnostic.json`，不询问人工 ACCEPT、不补调 mutation；适用 `semantic_content_v1` 时，诊断从生产 `page_results[*].semantic_content_stages` 保存 source→transformed 与 transformed→target 的完整性、check、摘要 hash 和有界 mismatch path，且不保存标题、正文、style 值或 raw XML。只有生产报告同时满足 `verified=true`、`lossless=true`、`copy_contract_satisfied=true` 且已非永久删源后，才要求用户检查 exact target 标题/内容并输入 run-bound `ACCEPT ... MovePageContent`。Agent、pytest、CI 或后台任务不得执行上述两条真实命令。
