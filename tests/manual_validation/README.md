...
| `delete` | Delete-Sandbox 下准备两个叶子 Page、Section 与 SectionGroup target；以测试 Batch effective Page 上限 3 证明 Notebook 总 Page 数超限时两个叶子 Page 仍可同批非永久 Delete，并证明含四 Page 的 Section scope 在 mutation 前拒绝且 snapshot 不变；三类成功批次均核对 `final_hierarchy` 与回收站状态；永久删除关闭；`--keep-worksite` 保持 Notebook 打开并记录精确目标 |
