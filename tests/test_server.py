...
def test_health_check_includes_runtime_diagnostics(monkeypatch):
    def fake_domain_items(include_recycle_bin=False):
    ...
    monkeypatch.setattr(server, "_domain_items", fake_domain_items)
...
def test_resolve_identifier_returns_single_item(monkeypatch):
    ...
    def fake_resolve_resource(identifier, item_type=None):
        assert identifier == "NB/Sec"
        assert item_type == "section"
        return expected

    monkeypatch.setattr(server, "_resolve_resource", fake_resolve_resource)
...
def test_open_hierarchy_resolves_existing_friendly_path_without_bridge(monkeypatch):
    ...
    def fake_find_resource_path(path, item_type=None):
    ...
    monkeypatch.setattr(server, "_find_resource_path", fake_find_resource_path)
    monkeypatch.setattr(server, "_bridge", fake_bridge)
...
def test_delete_hierarchy_retries_when_same_path_reappears_with_new_id(monkeypatch):
    ...
    monkeypatch.setattr(server, "_resolve_resource", lambda identifier, item_type=None: initial)
