...
async def copy_page(
    ...
    """With Create and Writes, copy one exact Page using capability-aware fidelity checks; the source title is preserved exactly unless destination_title is supplied, and Page titles never use filesystem cleaning; an existing first-level Page with the same or case-folded title is not a collision because the target is always a fresh allocated Page ID; include_subpages=false selects only the root and true selects its complete indentation subtree; OneNote chooses the reported destination position."""
...
async def move_page(
    ...
    """With Create, Writes, and Deletes, reconstructively move one Page only after capability-aware Copy verification; the source title is preserved exactly unless destination_title is supplied, and Page titles never use filesystem cleaning; an existing first-level Page with the same or case-folded title is not a collision because the target is always a fresh allocated Page ID; include_subpages=false protects excluded descendants and true moves the complete indentation subtree; OneNote chooses the reported destination position."""
