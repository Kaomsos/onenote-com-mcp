...
本设计采用一次性 cache schema 切换，不兼容旧 64-hex fingerprint/full-instance 目录。切换前由用户使用当前 human-gated `clear all` 流程清理旧 cache 与历史 runs；新 runtime 只接受空的受管 validation root 或新 schema，发现旧布局时直接 fail closed。
...
| 旧 schema 残留 | 要求回到升级前版本，通过既有 human-gated `clear all` 完成清理；新 runtime 不迁移或删除旧路径 |
...
本设计不提供旧 64-hex fingerprint/full-instance 目录的 lookup、迁移或 maintenance 兼容。切换过程固定为：

1. 用户在仍支持旧 schema 的当前版本中，先 dry-run 审查既有受管目标；
2. 用户本人在交互式前台显式运行既有 human-gated `clear all`，清理旧 cache 与历史 runs；
3. 只有清理 receipt/summary 表明目标全部成功且 `.local-validation/` 中没有旧 payload 后，才切换到新 schema 实现；
4. 新 runtime 只创建和读取短键布局；发现旧 marker、旧 index schema、64-hex fingerprint 目录、full-instance 目录或未清理的旧 run metadata 时直接 fail closed，并要求回到升级前版本完成用户清理；
5. 新 runtime 不自动移动、重命名、覆盖或删除任何旧路径，也不增加绕过参数。
