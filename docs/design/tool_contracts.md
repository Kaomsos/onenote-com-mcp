...
Copy/Move 是内容与拓扑级重建，不继承 OneNote-owned 修订/审计元数据。Page 转换仍排除 source authorship/revision marker 以及 `creationTime`、`lastModifiedTime` 等 volatile 时间字段；Section、SectionGroup、Notebook 目标也以新建对象的时间元数据为准。Page 根 `dateTime` 是例外：planning 从已读 source XML 冻结范围内每页的 UTC 自然秒，在正文/标题写入与 reorder 之后按 exact target ID 写入 dateTime-only payload，再用最终完整 target XML 重生成 acceptance，并对范围内 exact source Page 做一次有界重读。同一 UTC 秒（截断到秒、无 ± 容差；时区或小数秒不同仍通过）视为保真。范围内任一 Page 缺失、naive 或非法 `dateTime` 会在首个 mutation 前 fail closed，不进入 `copy_report`，也不补 now / 父对象 / 邻页。`page_results[*].date_time.status` 仅可能为 `verified`、`write_failed`、`readback_mismatch` 或 `source_drifted`；公开响应、异常和 audit 不含原始或规范化时间。空容器没有 dateTime 工作。`verified`、`lossless` 与 `copy_contract_satisfied` 覆盖下述受支持标题、内容对象、拓扑投影，以及范围内全部 Page 的 `date_time.status=verified`；它们仍不表示 source revision marker、原始创建时间或原始修改时间已保真。产品层边界见[产品能力边界](../product/README.md)；用户确认通过的正向与跨秒 Move 负向真实证据见已完成的 [TODO 043](../todo/043_copy_move_source_timestamp_fidelity.md)。
...
所有常规 verification tier 的 `page_results[*].title_readback_stages` 都独立返回 content-free 的 source→transformed 与 transformed→target title check。Plan 显式记录调用者是否请求 `destination_title`，不得用转换后的标题是否不同反推意图；默认模式（以及请求值与 source metadata title 相同的显式模式）要求 source XML title 与 source metadata 一致且 transformed title 原样保留。若 Page root 的显式 `destination_title` 实际改变标题，Copy/Move 先以源 Title 创建并写入内容，再在这个 exact allocated target ID 上复用 `rename_page` 的标题写入与收敛流程；Copy/Move 不使用、也不校验该 rename 流程返回的 Page ID。此时最终 target XML Title 不再要求与 Copy 的 transformed XML Title 精确相同：最终验证的 `equivalence.verification_tier` 是 `visible_text_projection`，`acceptance_checks` 仅为 `visible_text`，并且不执行 canonical XML、semantic-content 或其他内容对象的严格对比。`title_readback_stages` 仍要求回读的标题可见文字等于请求值。任一 stage 失败都会使该 Page `lossless=false`，进而阻止 Copy contract 和 Move 删源。`semantic_content_stages` 继续只属于未启用该标题改写路径的 `semantic_content_v1` 完整内容投影，不被扩展到 DisplayEquation 等独立 tier。
