...
本 TODO 是仓库切换为公开可见之前的硬发布门。品牌与 Demo、双语入口和开发文档、社区协作规范、来源署名与 relicense、Git 历史整理、原作者通知以及发布安全检查必须全部有可复核证据；只准备草稿、只存在 `LICENSE` 文件或只完成代码测试均不足以标记为完成。
