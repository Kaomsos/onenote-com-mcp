...
当前生产 Registry 仍注册 **56 个默认 typed 工具**。本 TODO 已在对话中冻结一版供后续实现和用户测试验收的 **v1.0 目标方案：默认 User profile 52 个工具**。本页是发布规划，不表示名称、数量、权限或返回格式已经落地；当前可执行合同仍以 [公开 Tool 契约](../design/tool_contracts.md) 和源码 Registry 为准。

目标数量的来源是：

```text
当前 56 - 5 个转为 Internal & Incubating + 1 个 launch_onenote_gui = 52
```

用户已于 2026-08-15 最终批准本文记录的 v1.0 发布方案并授权提交。本文所对应的“方案确认”目标已经完成；本 TODO 继续保持“进行中”，因为 Registry 实施、真实 OneNote 验收和实现后的用户测试准入批准仍待后续完成。
...
## v1.0 目标 User profile：52 个工具

### 1. Session（2）

- `health_check`
- `launch_onenote_gui`（新增，当前未实现；详见 [TODO 031](031_start_onenote_desktop_tool.md)）
...
`launch_onenote_gui` 是唯一目标名称，不提供 `start_onenote_app` alias。
...
- 本 TODO 当前只冻结方案，不在本次文档变更中修改生产代码；
...
- [x] 用户授权把当前方案写入产品规划文档；
- [ ] 默认 Registry 按冻结清单实现，隐藏/禁止入口无旁路；
- [ ] 名称、schema、description、effect、policy 和 envelope 全部统一；
- [ ] 自动化 Registry/schema/policy/文档投影合同与完整 pytest 通过；
- [ ] 适用 mutation 具有与当前实现一致的具名 scenario 和用户真实隔离证据；
- [ ] README、design、health、manual-validation 与实际 `tools/list` 一致；
- [ ] 用户最终审阅实现快照和证据，并明确批准开始用户测试。

## 完成证据记录

| 证据 | 结果/位置 |
| --- | --- |
| 当前 Registry 基线 | 当前 56；待实施前重新生成快照。 |
| v1.0 User profile | 本文“52 个工具”章节。 |
| 重命名与 Internal catalog | 本文对应章节。 |
| 授权矩阵 | 本文“7 个开关”章节。 |
| 最终 `tools/list` 名称/schema 快照 | 待实现。 |
| Forbidden/旁路/权限合同 | 待实现。 |
| 聚焦测试与完整 pytest | 待实现。 |
| mutation 用户真实证据 | 待实现后由用户执行并确认。 |
| 产品文档同步审查 | 规划态文档已开始对齐；实现态同步待完成。 |
| 实现后用户测试准入批准 | 待 Registry、合同、测试与真实证据完成后单独授权。 |
| v1.0 方案最终批准 | 2026-08-15 用户确认方案与文档修改通过并授权提交；实现后准入批准仍单独进行。 |

## 关联

- [公开 Tool 契约](../design/tool_contracts.md)：当前已实现工具面的 canonical source；实现后切换到本方案。
...
- [TODO 031](031_start_onenote_desktop_tool.md)：`launch_onenote_gui` 的目标合同与验收边界。
