...
| 037 | [用户测试期工具调用与实现体验优化](037_user_testing_experience_feedback_and_optimization.md)                          | 进行中 | P0     | UT-004 的同名 batch 与 `sort_children` 已完成自动化合同、9 个具名允许路径及 `create`/`reorder-page` mutation 前拒绝探针的 human-gated 验收；UT-001、UT-002、UT-003、UT-005、UT-007、UT-008、UT-009 与 UT-010 已完成，UT-006 仅记录 online-backed Notebook 的同步观察；UT-010 的五个 Page 工具已统一为 `include_subpages`，Delete/Reorder 子页保护及 Page Reparent/Delete batch 整批规划均有 fresh disposable 真实证据；整体代表性任务矩阵尚未宣告收尾。 |
