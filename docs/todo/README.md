...
| 031 | [显式 `launch_onenote_gui` 工具](031_start_onenote_desktop_tool.md)                                                  | 进行中 | P1     | 已冻结无参数、UI Control 授权、最多一次 process launch 与有界 GUI readiness 合同；当前仍未实现，`health_check` 保持 check-only。                                          |
...
| 034 | [用户测试前 MCP 工具发布面收敛](034_pre_user_testing_tool_surface_convergence.md)                                     | 进行中 | P0     | v1.0 方案已获用户最终批准：52 个 User 工具、13 项重命名、5 个非注册 Internal & Incubating 入口和 7 个默认关闭授权；等待实现、证据与实现后准入批准。                         |
...
| 036 | [Operation Runtime 操作执行控制层与 Tool 迁移](036_operation_runtime_control_plane_and_tool_migration.md)               | 已完成 | P1     | 56 项生产 Registry、五类 Runtime Strategy 与全 Tool 统一控制面已交付；自动化、dry-run 和用户真实 Read/Mutation/Copy/Move/effect/convergence 回归全部通过。                    |
