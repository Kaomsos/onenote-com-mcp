# 022：四层 Typed Metadata Query 与原生 Scope

> ID：022
> 状态：已完成
> 优先级：P1
> 类型：公开工具契约 / Query 能力 / Agent 可发现性
> 更新日期：2026-08-15
...
2026-08-14 随后将 Query fixture 提升为 recipe v4 的多项 Query 结构：两个 role 的 Notebook 根各有两个直属 SectionGroup 和两个直属 Section，主 Outer 下有两个 Inner Group，主 Inner 下有两个 Deep Section，主 Deep 中有两个 root Page 且 Parent 下有两个直接缩进 child；对应 start-node 和 `parent_page_id` case 均要求至少两个精确 fixture ID，不再用单结果代表集合行为。新增的 Deep/Root sibling `.one` 路径全部进入首个 role mutation 前预算。fixture 构建与 scenario 的打开 Notebook 基线现均通过 typed Query 完成。纯合同和完整自动化基线为 `952 passed`。

同日 `run-2026-08-14-22-32-01`、`22-36-13`、`22-37-18` 稳定证明 v4 cache template 的 `query-b/BChild` 只存在于关闭前 live snapshot，关闭后相同 template bytes 的三次 materialization 均连续 16 次只重绑 13/14 个对象；所有容器激活成功，回收站系统子树也已正确排除，因此不是 activation timeout。v5 将第二个 child 的 predecessor 从 Parent 改为第一个 Child，消除两个 reorder 对同一位置的竞争；同时 programmatic cache publish 在 exact close 前增加 lifecycle-owned `SyncHierarchy` 请求。新 recipe fingerprint 不再命中该 v4 template；这不引入 Query 的 close/reopen，也不恢复旧枚举工具依赖。
...
最终 schema 已删除 `query_hierarchy(resource_type,...,limit)`，新增四个固定资源类型入口；必填 discriminated `scope` 只用于后三者；`limit/truncated` 改为与 Search 同形的 `offset/page_size/has_more/next_offset`；Page 的重载 `parent_id` 拆为 `section_id/parent_page_id`；统一增加 `resource_type/query_kind/scope/pagination_consistency`。

新 fingerprint 的后续真实证据为 `run-2026-08-14-22-48-25`（`cache.decision=cold_build`）与 `run-2026-08-14-22-54-12`（`cache.decision=validated_hit`）。两次均为 `status=passed`，各记录 20 个受独立 hierarchy evidence 约束的 Query 请求，证明修复后的 fixture 可以稳定冷构建并复用 immutable cache。List 与层级浏览工具的后续产品重组不再属于本 TODO，由 [TODO 033](033_notebook_structure_list_and_expand_tools.md) 独立跟踪。

## 背景与实施前实现

实施前的默认 profile 只提供一个通用的 `query_hierarchy`：
...
`start_node_id` 必须按精确 COM ID 解析，拒绝名称、路径、Page ID、未知 ID、已关闭 Notebook 中的对象、错误类型和回收站中不可用的起点。start node 只界定向下查询的容器子树，不作为候选结果自动包含自身；因此以 SectionGroup 为起点的 `query_section_group` 只返回其后代 SectionGroup。
...
`offset/page_size` 组成与 `search_pages` 同形的无状态分页，但不是 COM scope、扫描预算或性能上限。它们不减少 `GetHierarchy` 输出大小和 Python 必须检查的候选数量。每一页都重新执行实时 hierarchy 读取，`pagination_consistency="live_hierarchy"`；返回顺序保留本次 COM snapshot 中相同资源类型的遍历顺序，不建立相关性排序或跨页冻结快照合同。无过滤 Query 可以按 `next_offset` 逐页取尽当前固定类型。
...
1. 已新增 `query_notebook`、`query_section_group`、`query_section`、`query_page`；
...
- Metadata Query 工具名只包含四个新 `query_*`，不包含 `query_hierarchy` 或 `global_query`；
