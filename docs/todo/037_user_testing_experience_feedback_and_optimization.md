...
UT-010 已接受：统一用 `include_subpages: bool = false` 取代所有公开 `page_scope`；`delete_page` 与 `reorder_page` 必须像现有 Page Move/Reparent 一样区分单页与完整缩进子树，并在单页路径中保护未选中的后代。Page Reparent/Delete 的 batch 必须基于整批 scope 一次性规划提升，不能逐项临时判断；当前均尚未实施。
...
| UT-010 | 2026-08-18 | 用户 / 删除或重排带缩进子页的父 Page，并统一 Page scope 参数 | Deletes / Writes；Reparent 另需 Organize | 当前 `delete_page` 与 `reorder_page` 未提供一致的单页/子树选择与子页保护；现有公开范围又使用字符串 `page_scope`；Page Reparent/Delete batch 若逐项判断提升还会受前项拓扑变化影响；`reliability` / `schema` / `capability` | 已接受：所有公开 `page_scope` 改为 `include_subpages: bool = false`；Delete/Reorder 单页路径提升并保护排除后代，完整子树路径按块处理；Page Reparent/Delete batch 从同一快照计算整批有效 scope 与一次性提升计划 | 当前仅完成代码/契约缺口核验；实现、自动化和用户 human-gated 复测均待完成 | accepted |
...
- **待实施与验证**：同步公开 Tool schema、description、Operation Registry、Service、响应、README/design/dev canonical 文档和所有 manual-validation 调用。确定性自动化至少覆盖五个受影响工具的 bool schema/default、root-only 提升成功/失败、完整子树、Reorder block、Delete leaf-to-root、整批 union/overlap/一次性提升、确认/预算/gate/回收站拒绝、partial failure 与零 replay。新增或扩展具名、human-gated manual-validation scenario，使用 fresh disposable 多层 Page 树，真实验证 Delete/Reorder 的 `false|true` 两种路径，并回归 Copy/Move/Reparent；真实执行只能由用户前台显式发起。完成前保持本项 `accepted`，不将 Delete/Reorder 描述为具有现有 Move/Reparent 同等后代保护。
