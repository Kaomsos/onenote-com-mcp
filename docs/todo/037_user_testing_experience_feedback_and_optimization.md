...
| --- | --- | --- | --- | --- | --- | --- | --- |
| UT-001 | 2026-08-16 | Grok Build / 读取 Page | 7 类 gate 关闭 | 未检查 health 或给出 GUI 恢复路径；`workflow` / `authorization` / `client` | 已优化 Session description、typed 恢复提示与 Grok 最小 UI Control 测试授权 | 用户前台确认拒绝后的恢复链通过；session-first health 与原始纯 read 任务仍待复测 | validating |
...
| UT-003 | 2026-08-16 | Grok Build / 浏览既有 Notebook 的 Page 缩进树 | Writes + UI Control | `expand_section`/`expand_hierarchy` 把真实存在的相邻 `page_level` 跳级当成整本 snapshot 非法；`reliability` / `response` / `capability` | L1 后跟随的 L3 直接映射为该 L1 的子节点；本轮只改文档 | 用户前台 query 已定位唯一跳级点；对象模型/契约已区分现状与已接受模型；代码未改 | accepted |
...
- **用户真实复测证据**：用户于 2026-08-16 提供前台截图并确认 Agent 按预期行动。当前任务是创建 disposable Notebook；可见 content-free 轨迹为 `Create Notebook ×1（GUI absent 后拒绝）→ Health Check ×1 → Launch OneNote GUI ×1 → Health Check ×1 → Create Notebook ×1（成功）`。这证明 Grok 能在 typed precondition failure 后发现并完成显式恢复链；截图未显示 error envelope 的完整字段，因此不从截图单独声称 `backend_calls` 数值。
- **剩余真实验证**：首次调用仍是授权 effect，而不是 session 开始先 `health_check`；本次任务也不是原始 Page read。UT-001 因而继续保持 `validating`，仍需以前台自然语言 Page read 验证 proactive session health 选择，并分别确认 GUI ready/absent 时纯 read 都不被 GUI effect 门限拒绝。
...
- **实现位置**：本轮只改文档，不改生产代码、测试或 fixture。对象模型与 Expand 合同已在 `docs/design/object_model.md`、`docs/design/tool_contracts.md`、`docs/design/hierarchy_parser.md` 区分**当前实现**与**已接受模型**。代码落地仍待后续变更：`src/local_onenote_mcp/services/hierarchy.py` 的 `_validate_hierarchy_snapshot` / `_browsing_snapshot` 范围、共享 indentation builder，以及 `tests/test_hierarchy_browsing.py` 中把 L1→L3 当作失败的合同。
- **自动化证据**：无。当前单测仍把 `pageLevel=1` 后接 `pageLevel=3` 作为 `expand_hierarchy` 的预期失败。
- **用户真实复测证据**：用户前台只读会话已确认：`expand_section`/`expand_hierarchy` 在真实相邻 `page_level` 跳级上失败，且错误 Section ID 不是请求的 root；`query_page` 可定位该相邻跳级点。本项尚未改代码，因此没有修复后的真实 Expand 复测。
- **合同影响**：已接受的模型改变了文档中对相邻 `page_level` 间隙的解释，但**尚未**改变运行中的 Expand fail-closed 边界。不改变 52 项工具面、七类 gate、exact ID 目标或 mutation 权限。实现落地时必须同步自动化测试，并由用户前台复测 Expand。
