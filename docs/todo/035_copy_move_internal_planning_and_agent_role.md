...
- [x] 列出四个 Plan Tool 当前提供的全部安全证明、公开字段、调用方和 manual-validation 依赖；
- [x] 为每项能力确定迁移后的内部责任归属和外部可观察保证，但不在本 TODO 中规定类或状态机实现；
- [x] 冻结七个 Copy/Move 执行工具移除 `plan_digest` 后的参数、confirmation、返回和失败合同；
- [x] 冻结本轮 Preview 决策：不交付任何 Preview 名称、profile 或 schema，所有生产注册路径均不可见；
- [x] 用户审阅并确认 Agent 职责边界、默认工具面和 Preview 高级分类。

### B. 收敛公开工具面

- [x] Copy/Move 执行工具改为单次公开调用，Agent 不再传递计划状态；
- [x] 从所有生产 registry/profile 移除四个 Plan Tool 和相关公开 adapter；
- [x] 删除 `plan_digest` 公开输入、Tool description、示例和 Agent 工作流；
- [x] 内部规划能力继续服务 Copy/Move，且不存在可枚举的 Plan 旁路；
- [x] Preview 本轮不实施；唯一生产 profile 与 Raw XML 环境变量旁路均有负合同证明不可见；
- [x] 不保留旧名 alias、兼容 wrapper 或可绕回旧两阶段协议的 generic/raw 工具。

### C. 同步验证与文档

- [x] 更新 `docs/design/tool_contracts.md`、架构文档、根 README、`health_check` 和工具目录；
- [x] Manual-validation Scenario 改为调用一次公开 Copy/Move Tool，同时继续保存内部安全结果的 content-free 外部证据；
- [x] 更新 scenario 最小 allowlist，移除 Plan Tool；Preview 不得成为 mutation scenario 的必需工具；
- [x] 更新注册/schema/description/Agent 调用链测试和 Copy/Move 聚焦合同；
- [x] 搜索并清理当前契约中的旧 Plan Tool、`plan_digest` 和强制两阶段调用说明；历史 TODO 中作为既有证据的名称保持历史语境。

## 自动化合同

至少覆盖：

- [x] 默认与所有生产 Profile 均不枚举四个 Plan Tool；
- [x] 七个 Copy/Move 执行 Tool 的 schema 不包含 `plan_digest` 或替代性跨调用状态 token；
- [x] 每个 Copy/Move 用户任务只需一次公开 mutation Tool 调用；
- [x] Copy/Move 在 execute 前仍拒绝 stale source、stale destination、错误 scope、错误 parent 和预算超限；
- [x] allocated/resolved/remapped identity、完整单射 ID map 和 fidelity 合同不回退；
- [x] Move 在 Copy 未完整验证、源重校验失败或目标状态不确定时绝不删除源；
- [x] partial/indeterminate、replay safety、completed steps 和 recovery 信息不因移除 Plan Tool 而丢失；
- [x] Preview 本轮明确不可见，唯一生产 profile、Raw XML 环境变量旁路与 health capability 均有负合同；
- [x] 本轮没有 Preview 实现，因此不存在 mutation/lifecycle bridge、执行 token 或内容泄漏路径；
- [x] Copy/Move policy 仍相互独立、默认 fail closed；不存在可影响 mutation 权限的 Preview exposure；
- [x] Registry、README、design、health capability 和 manual-validation allowlist 投影一致；
- [x] 聚焦纯测试与完整 `.venv\Scripts\python.exe -m pytest -q` 通过。
...
- 生产 profile 与 Raw XML 环境变量旁路都无法枚举 Plan 或 Preview；本轮没有 Preview mutation/lifecycle 路径可改变 fixture。
...
- [x] 四个 Plan Tool 已从所有生产工具面、公开 schema 和推荐调用链移除，且不存在兼容 alias 或注册旁路；
- [x] 七个 Copy/Move 执行工具均为单次公开调用，Agent 不再提交 `plan_digest` 或其他操作状态 token；
- [x] 现有 Plan 的全部安全能力已进入受控操作边界，并通过外部合同证明没有回退；
- [x] Agent 的意图/确认职责与服务的执行状态/安全证明职责已写入 canonical Tool contract 和 README；
- [x] 本轮不交付 Preview，公开文档与 health capability 明确其不存在；
- [x] 自动化 registry/schema/policy/Copy/Move/Preview 负合同与完整 pytest 通过；
- [x] Manual-validation 最小 allowlist 和 Scenario 已迁移到单次调用，用户确认适用 fresh/cache 真实回归通过；
- [x] TODO 034 的 Copy/Move 当前基线、design、README、health 和实际 `tools/list` 一致；TODO 034 其余 exposure 决策保持独立待办；
- [x] 用户确认最终 Copy/Move 工具集和 Agent 能力定位后，本 TODO 才可标记为“已完成”。

## 当前实施证据（2026-08-15）

- 生产 `tools.copying.TOOLS` 精确为七个单次调用；canonical Registry 的唯一 56 项生产集合不含四个 Plan 名称或 Preview 名称。
- 七个公开 schema 均无 `plan_digest/operation_id/token`；Service 在同一次调用中建立内部计划，`copy_report.planning` 只输出 content-free 摘要。
- Copy/Move、Server、Runtime、container reorder 与 mutation-control 聚焦回归通过 227 项；manual-validation 纯合同通过 574 项；完整 `.venv\Scripts\python.exe -m pytest -q` 通过 1022 项。跨 production/manual catalog 的覆盖断言已上移到顶层 Runtime 测试，故不计入 manual-validation 自身测试数；新增一项纯合同证明 accepted-but-not-complete 不会被共享 client 误判。
- `run.py all --dry-run` 与 `run.py all --use-cache --dry-run` 的 18 个批处理场景均全部通过；这只证明静态计划和安全边界，不替代用户真实 OneNote 证据。
- 七个 Copy/Move 的独立 `--use-cache --dry-run` 也均通过，并显示各自独立 policy、allowlist 和 lifecycle plan；dry-run 不读取 cache 或 OneNote。
- 七个具名 Copy/Move Scenario 已改为一次公开 mutation 调用，静态 allowlist 不含 Plan/Preview，并保存 `internal-planning.json`。
- 用户已确认本 TODO 适用的 fresh/cache 真实回归通过，并明确批准最终保留七个单次调用 Copy/Move Tool、不提供公共 Plan 或 Preview；Agent 只负责选择 Tool、typed ID、范围与 confirmation，内部 planning、执行证明、ID map、重放状态和恢复信息均由服务端负责。至此实现、自动化、真实验证和产品职责决策全部闭合，本 TODO 状态更新为“已完成”。
