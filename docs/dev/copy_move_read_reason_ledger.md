...
| `destination_precondition` | hierarchy | 确认 destination 父级、重名冲突、可创建位置，以及共享创建服务的父级/已分配 ID 前置条件。 | 无 mutation 时可复用已有 hierarchy snapshot；创建前的独立 service 读取仍带此 reason。 |
...
| `topology_verification` | hierarchy | 排序 XML 构造所需的当前目录、Copy target 的父级/Section/order/pageLevel 收敛，以及 partial outcome 的目标位置投影。 | 每次 `update_hierarchy` 后重新读取；同一无 mutation 验证阶段可复用。 |
...
| `delete_confirmation` | hierarchy | Move 删除前确认 source 或 root-only Page 子树、保留 descendant 的 promotion 前置，以及 promotion 的排序 XML 构造。 | 删除/提升前 live evidence；任何随后 mutation 都使其失效。 |
| `delete_convergence` | hierarchy、Page | 删除或 descendant promotion 后，确认 source 已不活跃、保留 descendant 拓扑与 Page 内容稳定。 | 必须使用 mutation 后 live evidence；不复用删除前快照。 |
...
Page Move 在 Copy 成功后额外出现 `source_drift_revalidation`、`delete_confirmation` 与 `delete_convergence`。Section、SectionGroup 与 Notebook Copy/Move 复用同一表；容器路径会为其包含的每个 Page 重复 Page 级 evidence，但不改变 reason 含义。
...
- `tests/test_copy_readback_ledger.py` 冻结所有公开 Copy/Move 形态的 `(backend_operation, read_reason)` 预算，并额外覆盖共享 `MutationService` 的创建、删除内部回读。测试断言这些 hierarchy/Page 读取没有 `None` reason。
