...
`SyncHierarchy` 返回只能证明请求被 COM 接受，不能证明刚创建的 fixture 已提交到本地 Section 文件。只有静态声明 `requires_persistence_checkpoint` 的版本化 Recipe 才在 fresh/cold-build 完整 live validation 后、首次业务 mutation或 template publish 前执行一次 `CloseNotebook(force=false)`，随后从同一 exact working path 重开、逐级激活、按 typed address 重绑 structure/evidence ID，并再次运行完整 live validator。当前包括已真实验证的 `reparent-page` v3，以及等待下一轮真实复验的 `create` v3 和 `copy-section-group` v5；Create v3 只构建业务实际需要的 `Duplicate-Title-Target` Section，Copy SectionGroup v5 则在四个本来为空的 anchor Group 内各放一个无 Page 的 typed sentinel Section，防止空 Group 在关闭/发布前没有形成物理子树。检查点证据写入 `fixture-persistence-remap.json` 和 manifest 的 `fixture_persistence_checkpoint`；任一步失败都在 mutation 前 fail closed 并保留现场。Recipe version 变化生成新 fingerprint，旧 template 不会命中新合同；检查点仍不解析或修改 `.one` bytes，也不打开 cache template。
