...
- 文件级例外只有两类。第一类是 fixture cache runtime：它可以对已由 lifecycle 精确关闭、由本次 disposable recipe bundle 创建的 Notebook 目录执行不解析内容的 opaque byte-for-byte copy；也可以清理由自身 marker 管理的 `.local-validation/fixture-cache/` 根下单个精确 `(fingerprint, template_instance_id)` template/staging entry。第二类是用户本人在交互式前台终端显式运行 `run.py clear runs|cache|all`，并在后续提示中现场输入动作绑定确认值：它只能清理固定 `.local-validation/` 根下由 metadata/marker 证明 ownership 的精确 run root、cache entry/staging 或 legacy cache lease metadata，必须拒绝非交互/重定向 stdin、持有短时 open lock、以一次当前 OneNote 实际打开路径快照 fail closed、拒绝 reparse point/宽根/未知形状，并在删除前后写 root-level receipt/summary。只有完整 target evidence 已进入 durable summary 的 `deleted` receipt 才能自动收敛；pending/failed/unbound receipt 必须保留。Cache 空 scaffold 只能对 canonical fingerprint 下已证明为空的目录逐层 `rmdir`，不得递归扩大范围。确认值不得作为 CLI 参数。Agent、pytest、CI、hook、timer、watcher 和后台任务只能运行这些 maintenance action 的 `--dry-run`。两类例外都不授权普通 Scenario 自动删除 working Notebook/普通 artifact/失败现场，也不覆盖用户 Notebook 或任意外部路径；任一证明不完整时必须 fail closed。Cache 不维护 run working lease，物理独立 working copy 的 run-local lifecycle lease 不构成 cache cleanup 门禁。
